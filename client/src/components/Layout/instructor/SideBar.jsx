...
import { HelpCircleIcon, LogOutIcon, XIcon, ChevronLeftIcon, ChevronRightIcon } from 'lucide-react';
      ...
      >
        <div className="flex h-full flex-col overflow-y-auto">
          ...
          <div className="flex h-16 items-center justify-between px-6 backdrop-blur-sm bg-gray-800/90 sticky top-0 transition-all duration-300">
            <div
              className={`flex items-center ${collapsed ? 'justify-center w-full' : ''} transition-all duration-300`}
            >
            ...
            <div className="flex items-center">
              ...
              {!collapsed && (
                <button
                  onClick={() => setIsOpen(false)}
                  className="lg:hidden ml-2 transition-opacity duration-300"
                >
              ...
              {navItems.map((item, index) => {
                ...
                return (
                  <li key={item.name} className="transition-all duration-300">
                    ...
                    >
                      <item.icon
                        className={`${collapsed ? '' : 'mr-3'} h-5 w-5 transition-all duration-300`}
                      />
                      {!collapsed && (
          ...
          {/* Footer */}
          <div
            className={`${collapsed ? 'px-2' : 'px-3'} py-4 border-t border-blue-600/50 transition-all duration-300`}
          >
            <ul className="space-y-1">
              ...
              <li className="transition-all duration-300">
                ...
                >
                  ...
                  />
                  {!collapsed && (
                    <span onClick={handleSignOut} className="transition-opacity duration-300">
                      Log Out
                    </span>
                  )}
                </a>
