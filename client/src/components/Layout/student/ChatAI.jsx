...
const ChatAI = () => {
  ...
  return (
    <div
      className={`fixed bottom-5 right-5 z-50 transition-all duration-300 ease-in-out ${isOpen ? 'w-96 md:w-[450px] h-[550px]' : 'w-16 h-16'}`}
    >
      {isOpen ? (
          ...
          <div className="chatAI__body flex-1 p-4 overflow-y-auto bg-gray-50">
            <div className="chatAI__body__messages space-y-4">
              {messages.map((message, index) => (
                <div
                  key={index}
                  className={`chatAI__body__messages__message flex ${message.isUser ? 'justify-end' : ''}`}
                >
                  <div
                    className={`chatAI__body__messages__message__text p-3 rounded-2xl max-w-[85%] shadow-sm ${
                      message.isUser
