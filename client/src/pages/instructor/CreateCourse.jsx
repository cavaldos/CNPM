...
const CreateCourse = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (
      !courseTitle ||
      !courseDescription ||
      !(courseCategory || (isCustomCategory && customCategory))
    ) {
    ...
    setIsLoading(true);
    navigate(`/courses/123/add-lessons`);
    // try {
    //   // Gọi API tạo khóa học từ service
    //   alert("Đang tạo khóa học mới...");
    ...
    //     alert(response.data.courseId);
    //     navigate(`/instructor/courses/${response.data.courseId}/add-lessons`);
    //   } else {
            ...
            {thumbnailType === 1 && (
              <Box sx={{ mt: 2, textAlign: "center" }}>
                ...
                <Typography variant="caption" display="block" sx={{ mt: 1 }}>
                  Khuyến nghị sử dụng ảnh với tỷ lệ 16:9, kích thước đề xuất
                  1280x720 pixels
                </Typography>
