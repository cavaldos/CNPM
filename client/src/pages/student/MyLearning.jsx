...
const MyLearning = () => {
  ...
  const handleCompleteCourse = async (e, enrollmentID) => {
    ...
    try {
      ...
      const response = await StudentService.enrollment.updateEnrollmentStatus(
        enrollmentID,
        'Completed'
      );
  ...
  const filteredCourses = courses.filter(course =>
    activeTab === 'enrolled' ? course.status === 'Enrolled' : course.status === 'Completed'
  );
  return (
        ...
        {!loading && !error && filteredCourses.length === 0 && (
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
            <p className="text-gray-600">
              {activeTab === 'enrolled' ? noEnrolledText : noCompletedText}
            </p>
          </div>
        )}
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex gap-4">
                    ...
                    />
                    <div>
                      <p className="text-sm text-gray-600 mb-1">{course.topic}</p>
